...
## 7.8.2 ’è”‚Íƒ~ƒ…[ƒ^ƒuƒ‹‚ÈƒIƒuƒWƒFƒNƒg‚É’ˆÓ‚·‚é
...
# •¶š—ñ‚ğ”j‰ó“I‚É‘å•¶š‚É•ÏX‚·‚é
Product::NAME.upcase!
Product::NAME

# ”z—ñ‚ÉV‚µ‚¢—v‘f‚ğ’Ç‰Á‚·‚é
Product::SOME_NAMES << 'Hoge'
Product::SOME_NAMES

# ƒnƒbƒVƒ…‚ÉV‚µ‚¢ƒL[‚Æ’l‚ğ’Ç‰Á‚·‚é
...
class Product
    ...
    def self.names_without_foo(names = SOME_NAMES)
        # names‚ªƒfƒtƒHƒ‹ƒg’l‚¾‚ÆAˆÈ‰º‚ÌƒR[ƒh‚Í’è”‚ÌSOME_NAMES‚ğ”j‰ó“I‚É•ÏX‚µ‚Ä‚¢‚é‚±‚Æ‚É‚È‚é
        names.delete('Foo')
        names
    end
end
...
class Product
    # ”z—ñ‚Ífreeze‚·‚é‚ª’†g‚Ì•¶š—ñ‚Ífreeze‚³‚ê‚È‚¢
    SOME_NAMES = ['Foo', 'Bar', 'Baz'].freeze
end
# 1”Ô–Ú‚Ì—v‘f‚ğ”j‰ó“I‚É‘å•¶š‚É•ÏX‚·‚é
...
# mapƒƒ\ƒbƒh‚ÅŠe—v‘f‚ğfreeze‚µAÅŒã‚Émapƒƒ\ƒbƒh‚Ì–ß‚è’l‚Ì”z—ñ‚ğfreeze‚·‚é
SOME_NAMES = ['Foo', 'Bar', 'Baz'].map(&:freeze).freeze

## 7.9.1 ƒNƒ‰ƒXƒCƒ“ƒXƒ^ƒ“ƒX•Ï”

class Product
    # ƒNƒ‰ƒXƒCƒ“ƒXƒ^ƒ“ƒX•Ï”
    @name = 'Product'
    ...
    # attr_reader:name‚Å‚à‚¢‚¢‚ªA@name‚Ì’†g‚ğˆÓ¯‚·‚é‚½‚ß‚É‚ ‚¦‚Äƒƒ\ƒbƒh‚ğ’è‹`‚·‚é
    def name
        # ƒCƒ“ƒXƒ^ƒ“ƒX•Ï”
        @name
    end
...
class DVD < Product
    ...
    def self.name
        # ƒNƒ‰ƒXƒCƒ“ƒXƒ^ƒ“ƒX•Ï”‚ğQÆ
        @name
    end

    def upcase_name
        # ƒCƒ“ƒXƒ^ƒ“ƒX•Ï”‚ğQÆ
        @name.upcase
    end
end
...
# DVDƒNƒ‰ƒX‚ğ’è‹`‚µ‚½ƒ^ƒCƒ~ƒ“ƒO‚Å@@name‚ª"DVD"‚É•ÏX‚³‚ê‚é
...
# Product.new‚Ìƒ^ƒCƒ~ƒ“ƒO‚Å@@name‚ª"A great movie"‚É•ÏX‚³‚ê‚é
Product.name
DVD.name
...
# DVD.new‚Ìƒ^ƒCƒ~ƒ“ƒO‚Å@@name‚ª"An awesome film"‚É•ÏX‚³‚ê‚é
product.name
Product.name
DVD.name


### 7.9.3 ƒOƒ[ƒoƒ‹•Ï”‚Æ‘g‚İ‚İ•Ï”

# ƒOƒ[ƒoƒ‹•Ï”‚ÌéŒ¾‚Æ’l‚Ì‘ã“ü
$program_name = 'Awesome program'

# ƒOƒ[ƒoƒ‹•Ï”‚ÉˆË‘¶‚·‚éƒNƒ‰ƒX
class Program
    def initialize(name)
        $program_name = name
    end
...
# $program_name‚É‚ÍŠù‚É–¼‘O‚ª‘ã“ü‚³‚ê‚Ä‚¢‚é
Program.name

program = Program.new('Super program')
program.name

# Program.new‚Ìƒ^ƒCƒ~ƒ“ƒO‚Å$program_name‚ª"Super program"‚É•ÏX‚³‚ê‚é
Program.name
$program_name

# $foobar‚ª–¢’è‹`‚Å‚ ‚ê‚Î(‚Â‚Ü‚èAˆê“x‚à‘ã“ü‚³‚ê‚Ä‚¢‚È‚¯‚ê‚Î)nil‚ª•Ô‚é
$foobar